...
def calculate_dermal_acute_toxicity(components, limits):
    ...
    for component in components:
        ...
        perc = component['percentage']
        print('dermal_cat',dermal_cat)
        if dermal_cat not in cat:
            continue
        if dermal_cat == "1":
            ATE = cat["1"]
        if dermal_cat == "2": 
...
def get_pictograms(hazard_no, img_src_actox, img_src_health_path, img_src_corr_path, img_src_warning_path):
    pictograms = set()

    for H in hazard_no:
        if H in ['H300', 'H301', 'H310', 'H311', 'H330', 'H331']:  # Acute toxicity
            pictograms.add(img_src_actox)

        if H in ['H340', 'H341', 'H350', 'H351', 'H360', 'H361', 'H370', 'H371', 'H372', 'H373', 'H334']:  # Health hazard
            pictograms.add(img_src_health_path)

        if H in ['H314', 'H318']:  # Corrosive
            pictograms.add(img_src_corr_path)

        if H in ['H317', 'H320', 'H319', 'H316', 'H315', 'H312', 'H302', 'H332']:  # Warning
            pictograms.add(img_src_warning_path)
    
    pictograms = list(pictograms)

    if img_src_actox in pictograms:
        if img_src_warning_path in pictograms:
            pictograms.remove(img_src_warning_path)
...
def get_H_statements(H_statements, components, limits, hazard_data):
    hazard_no = []
    hazard_statement = []
    pictogram = []
    signal_word = []
...
def calculations(components, limits, hazard_data, img_dirs):
    img_src_actox, img_src_health_path, img_src_corr_path, img_src_warning_path = img_dirs
    ...
    hazard_no, hazard_statement, pictogram, signal_word = get_H_statements(H_statements, components, limits, hazard_data)
    ...
    if len(signal_word) > 1:
        ...
        if danger == False and warning == False:
            signal_word = None
    elif len(signal_word) == 1:
        print('signalword',signal_word, type(signal_word))
        signal_word = signal_word[0]
    else:
        signal_word = ""
    toxicity_data = {
    f"Acute Oral Toxicity Category": mixture_oral_toxicity,
    f"Acute Dermal Toxicity Category": mixture_dermal_toxicity,
    f"Acute Inhalation Toxicity (Gases) Category": mixture_inhalation_toxicity_gases,
    f"Acute Inhalation Toxicity (Vapours) Category": mixture_inhalation_toxicity_vapours,
    f"Acute Inhalation Toxicity (Mist/Dust) Category": mixture_inhalation_toxicity_mist,
    f"Mutagenicity Category": mixture_muta,
    f"Carcinogenicity Category": mixture_carc,
    f"Reproductive Toxicity Category": mixture_repr,
    f"Skin Sensitization Category": mixture_skinsen,
    f"Respiratory Sensitization Category": mixture_respsen,
    f"Specific Target Organ Toxicity (Single Exposure) Category": mixture_stotse,
    f"Specific Target Organ Toxicity (Repeated Exposure) Category": mixture_stotre,
    f"{'Skin Corrosion' if mixture_skin == 1 else 'Skin Irritation'} Category": mixture_skin,
    f"{'Serious Eye Damage' if mixture_eye == 1 else 'Eye Irritation'} Category": mixture_eye,
}


    result_imgs = list(set(get_pictograms(hazard_no, img_src_actox, img_src_health_path, img_src_corr_path, img_src_warning_path)))
    
    st.session_state.results = {
        "components":components,
        "toxicity_data":toxicity_data,
        "pictograms":result_imgs,
        "H_codes": hazard_no,
        "H_statement":hazard_statement,
